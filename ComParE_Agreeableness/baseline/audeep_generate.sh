...
taskName="compare-Agreeableness"
...
# We use 20 ms Hann windows to compute spectrograms
window_width="0.20"

# We use 5 ms overlap between windows to compute spectrograms
window_overlap="0.05"

# Mel-scale spectrograms with 128 frequency bands are extracted
mel_bands="128"

# The ComParE Agreeableness audio files differ in length. By setting the --fixed-length option, we make sure that all
# audio files are exactly 10 second long. This is achieved by cutting or zero-padding audio files as required.
fixed_length="10"
...
parser="audeep.backend.parsers.compare_agreeableness.CompareAgreeablenessParser"
...
# Network training:
# =================
# Train for 8 epochs, in batches of 128 examples
num_epochs="8"
batch_size="128"
...
export_basename="ComParE_Agreeableness.auDeep"
