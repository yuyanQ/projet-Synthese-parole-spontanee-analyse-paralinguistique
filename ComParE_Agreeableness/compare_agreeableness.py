...
"""Parser for the ComParE Agreeableness dataset"""
import abc
...
_COMPARE_AGREEABLENESS_LABEL_MAP = {
    "A": 0,
    "PA": 1,
}


class CompareAgreeablenessParser(LoggingMixin, Parser):
    ...
    def _metadata(self) -> pd.DataFrame:
        if not self.can_parse():
            raise IOError("unable to parse the ComParE Agreeableness dataset at {}".format(self._basedir))
    ...
    def label_map(self) -> Optional[Mapping[str, int]]:
        if not self.can_parse():
            raise IOError("inable to parse the ComParE Agreeableness dataset at {}".format(self._basedir))

        return _COMPARE_AGREEABLENESS_LABEL_MAP
